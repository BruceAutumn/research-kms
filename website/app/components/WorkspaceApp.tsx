...
type Paper = {
  ...
  tags: string; favorite: number; reading_progress: number; created_at: string; updated_at: string;
};
type Note = {
  id: number; title: string; content: string; folder: string; properties: string; pinned: number;
  created_at: string; updated_at: string;
};
type Annotation = {
  id: number; paper_id: number; page: number; type: string; color: string; text: string;
  comment: string; rects_json: string; created_at: string; updated_at: string;
};
type AiMessage = { id: number; mode: string; role: "user" | "assistant"; content: string; created_at: string };
type Settings = { provider_name: string; base_url: string; model: string; protocol: string; updated_at: string; hasApiKey: boolean } | null;
type Workspace = {
...
export function WorkspaceApp() {
  ...
  return <div className="product-app">
    ...
    <div className="product-shell" style={{ gridTemplateColumns: `${navWidth}px 5px minmax(0,1fr)` }}>
      <nav className="product-nav" aria-label="工作区导航">
        ...
        <div className="nav-spacer" /><small>所有正文、附件、模型设置均按账户隔离</small>
      </nav>
      <ResizeHandle label="调整主边栏宽度" onDelta={delta => setNavWidth(navWidth + delta)} />
      <main className="product-content">
        ...
        {tab === "ai" && <AiPanel data={data} query={query} selectedPaperId={selectedPaperId} setSelectedPaperId={setSelectedPaperId} selectedNoteId={selectedNoteId} setSelectedNoteId={setSelectedNoteId} busy={busy} run={run} refresh={refresh} />}
...
function LibraryPanel({ data, query, selected, selectedId, setSelected, onUpload, busy, run, refresh, openAi, openVault }: {
  ...
  const [annotation, setAnnotation] = useState({ type: "highlight", color: "yellow", text: "", comment: "" });
  ...
  const patchPaper = (body: Record<string, unknown>, label = "paper") => selected && run(label, async () => {
    const response = await fetch(`/api/papers/${selected.id}`, { method: "PATCH", headers: jsonHeaders(), body: JSON.stringify(body) });
  ...
  const saveAnnotation = () => selected && run("annotation", async () => {
    ...
    if (!response.ok) throw new Error(await apiMessage(response)); setAnnotation(current => ({ ...current, text: "", comment: "" })); await refresh();
  });
...
function VaultPanel({ notes, query, selected, selectedId, setSelected, run, refresh }: {
  ...
  const save = () => run("note", async () => {
    if (!title.trim()) throw new Error("请填写笔记标题。"); setSaveState("保存中…");
    const response = await fetch("/api/notes", { method: "POST", headers: jsonHeaders(), body: JSON.stringify({ id: selected?.id, title, content, folder, properties: parseProperties(propertiesText), pinned }) });
  ...
  return <section className="vault-pro" style={{ gridTemplateColumns: `${treeWidth}px 5px minmax(400px,1fr) 5px ${linkWidth}px` }}>
    <aside className="vault-tree"><PaneTitle eyebrow="VAULT" title="知识库" action="＋" onAction={() => create()} /><div className="vault-search">⌕ {filtered.length} 个结果</div>{folders.map(name => <section key={name}><header><b>▾ {name}</b><button onClick={() => create(name)}>＋</button></header>{filtered.filter(note => (note.folder || "Inbox") === name).map(note => <button key={note.id} className={note.id === selectedId ? "active" : ""} onClick={() => setSelected(note.id)}><span>{note.pinned ? "★" : "◇"}</span>{note.title}</button>)}</section>)}</aside>
    <ResizeHandle label="调整文件树宽度" onDelta={delta => setTreeWidth(treeWidth + delta)} />
    ...
    <ResizeHandle label="调整关系边栏宽度" onDelta={delta => setLinkWidth(linkWidth - delta)} />
    <aside className="vault-links"><div className="context-head"><small>LINKED THINKING</small><b>关系与图谱</b></div><section><b>链接到 · {outgoing.length}</b>{outgoing.length ? outgoing.map(link => <button key={link} onClick={() => { const target = notes.find(note => note.title === link); if (target) setSelected(target.id); }}>→ {link}</button>) : <p>输入 [[笔记标题]] 创建链接</p>}</section><section><b>反向链接 · {backlinks.length}</b>{backlinks.length ? backlinks.map(note => <button key={note.id} onClick={() => setSelected(note.id)}>← {note.title}</button>) : <p>暂无反向链接</p>}</section><section><b>局部图谱</b><KnowledgeGraph notes={notes} selected={selected} onSelect={setSelected} /></section><section><b>数据库视图</b><div className="base-table"><span>名称</span><span>文件夹</span>{notes.slice(0, 6).map(note => <button key={note.id} onClick={() => setSelected(note.id)}><b>{note.title}</b><small>{note.folder}</small></button>)}</div></section></aside>
  </section>;
}
...
function ResizeHandle({ label, onDelta }: { label: string; onDelta: (delta: number) => void }) {
  const start = (event: React.PointerEvent<HTMLDivElement>) => { const origin = event.clientX; const move = (next: PointerEvent) => onDelta(next.clientX - origin); const end = () => { window.removeEventListener("pointermove", move); window.removeEventListener("pointerup", end); }; window.addEventListener("pointermove", move); window.addEventListener("pointerup", end); };
  return <div className="resize-handle" role="separator" aria-orientation="vertical" aria-label={label} onPointerDown={start}><span /></div>;
}
...
async function extractPdfText(file: File) { const task = getDocument({ data: new Uint8Array(await file.arrayBuffer()) }); const pdf = await task.promise; const pages: string[] = []; const maxPages = Math.min(pdf.numPages, 80); for (let number = 1; number <= maxPages; number += 1) { const page = await pdf.getPage(number); const content = await page.getTextContent(); pages.push(content.items.map(item => "str" in item ? item.str : "").join(" ")); if (pages.join("\n").length > 120_000) break; } return pages.join("\n").slice(0, 120_000); }

...
export default async function AdminPage() {
  ...
  const [users, papers, notes, messages, bytes, events] = await Promise.all([
    ...
    db.prepare("SELECT action, detail, created_at FROM audit_events ORDER BY id DESC LIMIT 50").all(),
  ]);
  return <main className="admin-page"><header><a className="brand" href="/app"><span>R</span><b>Research KMS</b></a><div><span>运营后台</span><a href="/app">返回工作区</a></div></header><section className="admin-content"><div className="admin-heading"><span>PRIVATE OPERATIONS</span><h1>系统概览</h1><p>只有站点管理员可访问。用户 API Key 和文献正文不在此处展示。</p></div><div className="metric-grid"><Metric label="用户" value={users?.count || 0} /><Metric label="文献" value={papers?.count || 0} /><Metric label="Vault 笔记" value={notes?.count || 0} /><Metric label="AI 消息" value={messages?.count || 0} /><Metric label="PDF 存储" value={formatBytes(bytes?.count || 0)} /></div><section className="audit-table"><div className="audit-head"><h2>最近审计事件</h2><span>{events.results.length} records</span></div>{events.results.length ? events.results.map((event, index) => { const row = event as Record<string, string>; return <div className="audit-row" key={`${row.created_at}-${index}`}><b>{row.action}</b><span>{row.detail || "—"}</span><time>{new Date(row.created_at).toLocaleString("zh-CN")}</time></div>; }) : <p className="admin-empty">暂无运行记录。</p>}</section></section></main>;
}

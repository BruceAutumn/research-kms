...
export async function POST(request: Request) {
  try {
    ...
    const body = await request.json() as { id?: number; title?: string; content?: string; folder?: string; properties?: Record<string, unknown>; pinned?: boolean };
    ...
    if (body.id) {
      const result = await productEnv().DB.prepare(
        "UPDATE notes SET title = ?, content = ?, folder = ?, properties = ?, pinned = ?, updated_at = ? WHERE id = ? AND user_id = ?",
      ).bind(title, content, folder, properties, pinned, now, body.id, user.userId).run();
      if (!result.meta.changes) throw new HttpError(404, "笔记不存在。");
      await audit(user.userId, "note.updated", title);
      return Response.json({ id: body.id, title, content, folder, properties, pinned, updated_at: now });
    }
    const result = await productEnv().DB.prepare(`
      INSERT INTO notes (user_id, title, content, folder, properties, pinned, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).bind(user.userId, title, content, folder, properties, pinned, now, now).run();
    await audit(user.userId, "note.created", title);
    return Response.json({ id: result.meta.last_row_id, title, content, folder, properties, pinned, created_at: now, updated_at: now }, { status: 201 });

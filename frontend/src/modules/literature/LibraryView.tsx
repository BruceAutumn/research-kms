...
export default function LibraryView({ newCollectionOpen, onNewCollectionClose }: LibraryViewProps) {
  ...
  // Command Palette"Search Papers"-> Focus search box
  useEffect(() => {
  ...
  // Full List: LeftsideTreecountand Tags Aggregate uses
  ...
  // Semantic/Mixed Mode: filter by search resultsandSort
  ...
  const selectionHandlers = useMemo(
    () => ({
      ...
      onDeletePermanently: async (paperIds: number[]) => {
        if (!window.confirm(`Permanent Delete ${paperIds.length} paperPaper(with disk PDF)? this action notCanResume. `)) return;
    ...
    <div className="lit-library">
      ...
      <div className="lit-toolbar">
        ...
        <div className="lit-toolbar-search">
          ...
          />
          {searchTerm && (
            <button type="button" className="icon-btn" title="Clear Search" onClick={() => setSearchTerm('')}>
          ...
          <div className="search-mode-toggle">
            ...
            <button
              ...
              title="Hybrid Search(Keywords + Semantic)"
    ...
    <div className="dialog-shell" role="dialog" aria-modal="true">
      ...
      <div className="dialog">
        <div className="dialog-header">
          <span className="dialog-title">Add Tag</span>
          <button type="button" className="icon-btn" aria-label="close" onClick={onClose}>
            <X size={14} />
          </button>
        </div>
        <div className="dialog-body">
          <p className="dialog-desc">For selected {targetPapers.length} paperPaperAdd Tag(commaSeparate): </p>
          <input
            className="field-input"
            autoFocus
            value={draft}
            placeholder="E.g.: cesium, adsorption"

...
/** Inline Edit: Click to input, Save on blur, Esc Cancel.  */
function InlineEdit({ value, placeholder, multiline, onSave }: InlineEditProps) {
  ...
  return (
    <button
      ...
      title={`Click Edit${placeholder ? ` ${placeholder}` : ''}`}
  ...
  if (!paper) {
    return (
      <div className="insp-empty">
        ...
        <p className="insp-empty-hint">in Papers Select a paper from list, thisinshow detailsandMetadata. </p>
      </div>
    );
  }
  ...
  return (
    <div className="insp">
      ...
      <Section title="Info" defaultOpen>
        <div className="insp-info">
          ...
          <div className="insp-field">
            ...
            <InlineEdit value={paper.authors || ''} placeholder="Not Filled" onSave={(v) => patch({ authors: v })} />
          </div>
      ...
      <Section title="Abstract" defaultOpen>
        <InlineEdit value={paper.abstract || ''} placeholder="No Abstract, Click Fill" multiline onSave={(v) => patch({ abstract: v })} />
      </Section>
        ...
        <div className="insp-tags">
          {(paper.tags || []).map((tag) => (
            <span key={tag} className="tag-chip">
              {tag}
              <button type="button" aria-label={`Delete Tag ${tag}`} onClick={() => removeTag(tag)}>
                <X size={10} aria-hidden="true" />
              </button>
            </span>
          ))}
          {paper.tags.length === 0 && <span className="insp-empty-hint">No Tags</span>}
      ...
      <Section title="Attachments" defaultOpen>
        {paper.pdfPath ? (
          ...
          <p className="insp-empty-hint">no PDF Attachment(DOI / BibTeX import)</p>
        )}
      </Section>
      ...
      <Section title="Notes" defaultOpen>
        <div className="insp-notes">
          <button type="button" className="btn btn-primary" onClick={() => setShowNoteDialog(true)}>
            ...
            Generate note to Vault
          </button>
          <p className="insp-empty-hint">Click Generate Note, Auto link to vault Vault</p>
        </div>
      </Section>
      ...
      <Section title="AI Metadata" defaultOpen>
        <div className="insp-ai">
          <div className="insp-ai-row">
            <span className="insp-label">state</span>
            <span className={`ai-chip ${aiMeta.cls}`}>{aiMeta.label}</span>
          </div>
          {extractions.length > 0 && (
            <div className="insp-ai-row">
              <span className="insp-label">Extraction Record</span>
              <span>{extractions.length}  </span>
            </div>
          )}
          {mock && <span className="mock-badge">Mock Output</span>}
      ...
      <Section title="AI Metadata . KV" count={metadataFields.length}
        actions={!kvEditing && metadataFields.length > 0 ? (
          <button type="button" className="icon-btn" title="Edit KV Metadata" onClick={() => { setKvDraft(metadataFields); setKvEditing(true); }}>
      ...
      >
        {kvEditing ? (
              ...
              <button type="button" className="btn" disabled={kvSaving} onClick={async () => {
                ...
                try {
                  ...
                  if (result.overwrittenKeys.length > 0) {
                    flashNote(`Saved, But detected duplicate field ${result.overwrittenKeys.join(', ')}, Only kept last filled value. `);
                  } else {
                    flashNote('KV Metadata saved');
                  }
                  setKvEditing(false);
                } catch (err) {
                  flashNote(`Save failed: ${getErrorMessage(err)}`);
                } finally {
                  setKvSaving(false);
                }
              }}>
          ...
          <div className="insp-kv-empty">
            <p className="insp-empty-hint">No KV Metadata</p>
      ...
      <Section title="Related" count={relatedQuery.data?.length}>
        {relatedQuery.data && relatedQuery.data.length > 0 ? (
          ...
          <p className="insp-empty-hint">same Collection / sameTagrelatedPaperwill show herein</p>
        )}
      </Section>
      {showNoteDialog && (
        <GenerateNoteDialog

...
export default function CollectionTree({
  ...
  function collectionMenu(id: number): MenuItem[] {
    return [
      ...
      },
      {
        ...
        onClick: () => {
          ...
          dispatchAiAction({
            ...
            instruction: `请分析 Collection「${collection?.name || id}」中的文献，输出主题聚类、关键论文和阅读/笔记计划。`,
      ...
      },
      {
        ...
        danger: true,
        onClick: async () => {
          if (window.confirm('删除该 Collection？其中的文献不会被删除。')) {
  ...
  // ---- 拖放：Paper → Collection（多对多，不复制物理数据） ----
  ...
  const fixedGroups: Array<{ key: string; label: string; icon: React.ReactNode; count?: number }> = [
    { key: 'all', label: '全部文献', icon: <Library size={13} />, count: counts.all },
    { key: 'recent', label: '最近添加', icon: <Clock size={13} />, count: counts.recent },
    { key: 'recently_read', label: '最近阅读', icon: <BookOpenCheck size={13} />, count: counts.recently_read },
    { key: 'favorites', label: '收藏', icon: <Star size={13} />, count: counts.favorites },
    { key: 'unread', label: '未阅读', icon: <FileQuestion size={13} />, count: counts.unread },
    { key: 'ai_processed', label: 'AI 已解析', icon: <Check size={13} />, count: counts.ai_processed },
    { key: 'ai_pending', label: 'AI 待解析', icon: <Sparkles size={13} />, count: counts.ai_pending }
  ];
  ...
  return (
    <div className="tree">
      <div className="tree-section-label">我的文库</div>
      ...
      <div className="tree-section-label tree-section-head">
        ...
        <button type="button" className="icon-btn" title="新建 Collection" onClick={() => onNewCollection()}>
          <Plus size={13} aria-hidden="true" />
        </button>
      </div>
      {tree.length === 0 && <div className="tree-empty">右键此处或点 ＋ 新建 Collection</div>}
      {tree.map(renderNode)}

      <div className="tree-section-label tree-section-head">
        <span>Tags</span>
      </div>
      {tagCounts.length === 0 && <div className="tree-empty">暂无标签</div>}
      ...
      >
        ...
        <span className="tree-row-label">废纸篓</span>

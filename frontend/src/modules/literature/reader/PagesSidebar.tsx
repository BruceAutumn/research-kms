  ...
  return (
    <div className="pages-sidebar">
      <div className="reader-subtabs" role="tablist">
        <button type="button" role="tab" aria-selected={tab === 'thumbnails'} className={`reader-subtab ${tab === 'thumbnails' ? 'is-active' : ''}`} onClick={() => setTab('thumbnails')} title="缩略图">
          <ImageIcon size={14} aria-hidden="true" />
        </button>
        <button type="button" role="tab" aria-selected={tab === 'outline'} className={`reader-subtab ${tab === 'outline' ? 'is-active' : ''}`} onClick={() => setTab('outline')} title="目录">
          <AlignLeft size={14} aria-hidden="true" />
        </button>
        <button type="button" role="tab" aria-selected={tab === 'annotations'} className={`reader-subtab ${tab === 'annotations' ? 'is-active' : ''}`} onClick={() => setTab('annotations')} title="标注">
      ...
      {tab === 'outline' && (
        <div className="outline-list">
          {outlineLoading && <p className="reader-hint">加载目录…</p>}
          {!outlineLoading && outline.length === 0 && (
            <p className="reader-hint">此 PDF 没有目录（outline）</p>
          )}
        ...
        <div className="annotation-list">
          ...
          {annotations.map((annotation) => (
            <div key={annotation.id} className="annotation-item">
              ...
              <button type="button" className="icon-btn annotation-delete" title="删除标注" onClick={() => onDeleteAnnotation(annotation.id)}>
...
function ThumbList({
  ...
  if (!pdf) {
    return <p className="reader-hint">PDF 加载中…</p>;
  }

...
export default function AiStudioModule() {
  ...
  // 全局搜索深链：/ai?conversation=N。加载后清掉参数，避免切换模块时重复打开。
  useEffect(() => {
    const raw = searchParams.get('conversation');
    if (!raw || conversationsQuery.isLoading) return;
    const id = Number(raw);
    ...
    // selectConversation 是函数声明；这里只在 URL 或会话列表变化时消费一次深链。
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [searchParams, setSearchParams, conversations, conversationsQuery.isLoading]);
  ...
  useEffect(() => {
    const applyAction = (action: AiAction) => {
      ...
      if (action.type === 'run-agent') {
        setMode('work');
        setInput(action.instruction || (action.label ? `请分析：${action.label}` : '请基于当前上下文执行科研助理任务。'));
  ...
  async function sendChat() {
    ...
    setInput('');
    // 发送后附件的生命周期交给后端；后端无论成功失败都会清理临时文件。
    setAttachments([]);
    ...
    await postChatStream({
      ...
      // 请求若根本没到后端，兜底删除尚未被后端接管的临时附件。
      void Promise.allSettled(sentAttachments.map((attachment) => deleteAiAttachment(attachment.path)));
  ...
  async function selectConversation(conversation: AiConversation) {
    ...
    setAttachments([]);
    void Promise.allSettled(staleAttachments.map((attachment) => deleteAiAttachment(attachment.path)));
    abortRef.current?.abort();
    eventSourceRef.current?.close();
    setRunning(false);
    setError(null);
  ...
  async function removeConversation(conversation: AiConversation) {
    if (!window.confirm(`删除会话「${conversation.title || '未命名对话'}」？`)) return;
  ...
  return (
    <div className="ai2-root">
      ...
      <main className="ai2-main">
        ...
        {models.length === 0 && <button className="ai2-no-model" onClick={() => { window.history.pushState({}, '', '/settings/models'); window.dispatchEvent(new PopStateEvent('popstate')); }}>未配置模型 · 去设置</button>}
        {/* 上下文 chips 放在输入框正上方：发送之前就要看得见喂进去了什么。 */}
        <ContextBar
          refs={contextRefs}
        ...
        <Composer
          ...
          onRemoveAttachment={(attachment) => {
            ...
            void deleteAiAttachment(attachment.path).catch(() => undefined);
          }}
        />
      </main>
      <AiSidePanel

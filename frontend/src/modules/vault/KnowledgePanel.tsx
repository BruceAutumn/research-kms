...
/** Right Panel Knowledge Panel: Icon Tab CutChange, not simultaneouslyExpand.  */
  ...
  if (!activePath) {
    return (
      <div className="vault-knowledge">
        ...
        <div className="vault-kp-empty">openOnepaperNoteafter, thisinshow Properties / Backlinks / Outgoing / Outline / Graph / AI. </div>
      </div>
    );
  }
  ...
  return (
    <div className="vault-knowledge">
      ...
      <div className="vault-kp-body">
        ...
        {panel === 'graph' && (
          <Suspense fallback={<div className="vault-kp-empty">Load Graph...</div>}>
...
function PropertiesPanel({ path, file, onSaved }: {
  ...
  async function save() {
    ...
    try {
      ...
      setInfo('alreadyWrite Back frontmatter. ');
      onSaved();
    } catch (err) {
  ...
  return (
    <div className="vault-props">
      {!file.frontmatterValid && (
        <div className="vault-fm-banner">! frontmatter parseFailed: Fix in source mode first YAML, Edit Again Properties(will not overwrite nowOriginaltext). </div>
      )}
      ...
      <div className="vault-props-list">
        {rows.map((row, index) => (
          <div key={`${row.key}-${index}`} className="vault-prop-row">
            ...
            <button type="button" className="icon-btn" title="Delete Property" onClick={() => removeRow(index)}>
              <Trash2 size={12} />
            </button>
          </div>
        ))}
      ...
      <div className="vault-prop-add">
        <input className="field-input" placeholder="New Property Name(Like paper / topic)" value={newKey}
          onChange={(event) => setNewKey(event.target.value)}
          onKeyDown={(event) => { if (event.key === 'Enter') addRow(); }} />
        <button type="button" className="btn" onClick={addRow}><Plus size={12} /> Add</button>
      </div>
      <p className="vault-kp-hint">add/delete/modify allSyncWrite Back .md   YAML Block; parseFailedwill not overwrite whenRaw Text. </p>
    </div>
  );
}
...
function BacklinksPanel({ path, onChanged }: { path: string; onChanged: () => void }) {
  ...
  return (
    <div className="vault-backlinks">
      ...
      {backlinksQuery.data && backlinksQuery.data.length === 0 && <p className="vault-kp-hint">No Linked mentions. </p>}
      ...
      {unlinkedQuery.data?.map((row) => (
        <div key={row.path} className="vault-backlink-row">
          ...
          <button type="button" className="btn vault-link-create-btn" disabled={busyPath === row.path}
            ...
            <Link2 size={12} /> Build Link
          </button>
        </div>
      ))}
      {unlinkedQuery.data && unlinkedQuery.data.length === 0 && <p className="vault-kp-hint">No Unlinked mentions(In body but unused [[]] wrappedTitle). </p>}
      {error && <div className="vault-error">{error}</div>}
    </div>
  );
}
        ...
        <div key={index} className={`vault-outgoing-row ${row.resolved ? '' : 'is-missing'}`}>
          {row.resolved ? (
            ...
            <>
              ...
              <span className="vault-kp-hint">notCreate -- Click to create note</span>
              {busyTitle === row.targetTitle && <span className="vault-kp-hint">Creating...</span>}
            </>
          )}
          {row.alias && <span className="vault-kp-hint">elseName: {row.alias}</span>}
        </div>
      ))}
      {outgoingQuery.data && outgoingQuery.data.length === 0 && <p className="vault-kp-hint">no out-links. </p>}
      {error && <div className="vault-error">{error}</div>}
    </div>
  );
}
...
function OutlinePanel({ path, content, onJump }: { path: string; content: string; onJump: (line: number) => void }) {
  ...
  return (
    <div className="vault-outline">
      <div className="vault-kp-section-title">Outline(Body Title)</div>
      {headings.length === 0 && <p className="vault-kp-hint">noTitle(use # / ## / ### writing). </p>}

...
/** from :root Take computed color(Cytoscape canvas not eat var(), Parse once at runtime).  */
function cssVar(name: string, fallback: string): string {
  const value = getComputedStyle(document.documentElement).getPropertyValue(name).trim();
  return value || fallback;
}

/** Folder Color Palette: All from :root Semantic token(Forbid hard write hex).  */
...
/**
 * Relationship Graph(Cytoscape.js)-- analysisTool, Not home animation. 
 * Node > 300 Auto degrade: transition animation, grid Layout, Hidden Tag. 
 */
export default function GraphPanel({ currentPath }: GraphPanelProps) {
  ...
  useEffect(() => {
    ...
    cy.on('tap', 'node', (event) => {
      ...
      if (id.startsWith('unresolved:')) {
        ...
        requestOpen(title); // notCreate -> Hint Create
        return;
      }
      requestOpen(id);
    });
  ...
  // Search Locate: Focus matched node and highlight neighborhood
  useEffect(() => {
    ...
    <div className="vault-graph">
      <div className="vault-graph-controls">
        ...
        <div className="lit-toolbar-search vault-graph-search">
          <Search size={12} />
          <input placeholder="Search and locate node..." value={query} onChange={(event) => setQuery(event.target.value)} />
        </div>
      </div>
      {data ? (
        <>
          <div className="vault-graph-stats">
            {data.stats.nodes} Node . {data.stats.edges} side
            {degraded && <span className="vault-graph-degrade"> . alreadyAuto degrade(transition animation/Simplified Layout)</span>}
            {mode === 'local' && !currentPath && ' . not openNote, Please select a note first'}
          </div>
        ...
        <div className="vault-graph-empty">
          <Network size={24} />
          <p>{mode === 'global' ? 'Load full vault graph...' : 'openOnepaperNoteafter show Local Graph. '}</p>
        </div>
      )}
    </div>
  );

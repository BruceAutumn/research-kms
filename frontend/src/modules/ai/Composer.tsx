  ...
  return (
    <div className="ai2-composer">
        ...
        <div className="ai2-attach-list">
          {attachments.map((att, i) => (
            <span key={i} className="ai2-attach-chip">
              ...
              <button type="button" className="ai2-attach-remove" onClick={() => onAttachmentsChange(attachments.filter((_, j) => j !== i))}>
                <X size={10} />
              </button>
            </span>
          ))}
      ...
      <textarea
        ...
        placeholder={mode === 'chat' ? 'askOneresearch questions...' : 'let Agent Execute a workflow...'}
      />
      <div className="ai2-composer-bar">
        <button className="icon-btn" title="Add Context" onClick={onAddContext}><Plus size={16} /></button>
        <button className="icon-btn" title="Upload File" onClick={() => fileInputRef.current?.click()} disabled={uploading}>
          <Paperclip size={15} />
        </button>
        ...
        <div className="ai2-segment">
          ...
          <button className={mode === 'work' ? 'is-active' : ''} onClick={() => onMode('work')}><Wrench size={14} />Work</button>
        </div>
        <button
        ...
        {showSettings && (
          <div className="ai2-effort-popover">
            <div className="ai2-effort-popover-title">Reasoning Effort</div>

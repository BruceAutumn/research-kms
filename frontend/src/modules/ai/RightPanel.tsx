...
export default function RightPanel({ agent, contextRefs, tools, runs, onHistory }: Props) {
  ...
  return (
    <aside className="ai2-right">
      <section><h3>Instructions</h3><p>{agent?.prompt || 'Select Agent after show systemPrompt. '}</p></section>
      <section><h3>Context</h3>{contextRefs.length ? contextRefs.map((ref, index) => <span className="ai2-chip" key={index}>{ref.label || ref.path || ref.type}</span>) : <p>No Context</p>}</section>

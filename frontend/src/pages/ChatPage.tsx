...
export default function ChatPage() {
  ...
  return (
    <div className="mx-auto flex h-[calc(100vh-4rem)] max-w-4xl flex-col space-y-4">
      <div>
        <h1 className="text-2xl font-bold">Paper Q&A</h1>
        <p className="text-sm text-slate-500">Optional link to paper;Backend injects paper full text before 12000 asContext. </p>
      </div>
      <select className="rounded border bg-white px-3 py-2" value={paperId || ''} onChange={(e) => setPaperId(e.target.value ? Number(e.target.value) : undefined)}>
        <option value="">Without paper context</option>
        {papers.map((paper) => <option key={paper.id} value={paper.id}>{paper.title}</option>)}
      </select>
      <StatusMessage error={error} info={loading ? 'AI Thinking...' : undefined} />
      <div className="flex-1 space-y-3 overflow-auto rounded-xl bg-white p-4 shadow-sm">
        ...
        {messages.length === 0 && <p className="text-center text-sm text-slate-500">Ask about your research papers. </p>}
      </div>
      <form className="flex gap-2" onSubmit={send}>
        <input className="flex-1 rounded border px-3 py-2" value={input} onChange={(e) => setInput(e.target.value)} placeholder="Input Question..." />
        <button className="rounded bg-indigo-600 px-4 py-2 text-white" disabled={loading}>Send</button>
      </form>
    </div>
  );
}

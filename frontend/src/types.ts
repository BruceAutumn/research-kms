...
export interface Paper {
  ...
  dateModified?: string;
  lastOpenedAt?: string;
  /** Zotero 式阅读分诊：unread / reading / done */
  readStatus?: 'unread' | 'reading' | 'done';
  /** 0-5 星，0 = 未评级 */
  rating?: number;
  /** 处理状态：PROCESSING / READY / ERROR / DUPLICATE */
...
export interface Annotation {
  ...
  paperId: number;
  page: number;
  /** legacy 坐标字段：Phase 4 前的前端只写这里。 */
  position?: string;
  /** V9 起的坐标字段，后端工具（ListAnnotationsTool / 笔记导出）读的是它。 */
  rectsJson?: string;
  selectedText?: string;
  color: string;
  comment?: string;
...
export interface LlmModel {
  ...
  isDefault: boolean;
  enabled: boolean;
  /** 'chat' | 'embedding' —— 决定这个模型能不能被 EmbeddingService 选中。 */
...
export interface PdfSelection {
  text: string;
  page: number;
  /** PDF 页面坐标系下的归一化矩形列表（viewToPage 换算后） */
...
export interface VaultTreeNode {
  name: string;
  path: string; // Vault 内相对路径（root 为 ''）
  type: VaultNodeType;
  mtime?: number;
  ctime?: number;

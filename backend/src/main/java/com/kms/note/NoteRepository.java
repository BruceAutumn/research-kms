...
public interface NoteRepository extends JpaRepository<Note, Long> {
    ...
    // ---- Phase 4: Vault Index cache query(path as standard) ----
    ...
    /** PostgreSQL tsvector Full-text Search(Title Weighted > Body).  */
    @Query(value = """
    ...
    /** Update tsvector(Chinese bigram token already in Java side generate).  */
    @Modifying
    @Query(value = """
            update notes
            set search_vector =

...
public class NoteTemplateService {
    ...
    public NoteTemplateDto update(Long id, NoteTemplateDto dto) {
        ...
        if (t.getIsBuiltin() && dto.body() != null && !dto.body().equals(t.getBody())) {
            throw new ApiException(HttpStatus.BAD_REQUEST, "Built-in template cannot be modified body");
        }
    ...
    public void delete(Long id) {
        NoteTemplate t = findTemplate(id);
        if (t.getIsBuiltin()) {
            throw new ApiException(HttpStatus.BAD_REQUEST, "Built-in template cannot be deleted");
        }

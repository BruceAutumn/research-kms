...
public class PaperController {
    ...
    public ResponseEntity<?> file(@PathVariable Long id, @RequestHeader HttpHeaders headers) throws IOException {
        ...
        // pdfjs 会使用 HTTP Range 分段请求 PDF。Spring 的 ResourceRegion 在某些
        // application/pdf 场景下没有可用 converter，所以这里显式解析 Range，返回 byte[]。
        // 这样逻辑更直白，也更适合 v1 学习项目阅读。
    ...
    public NoteDto exportAnnotationsToNote(@PathVariable Long id, @RequestBody java.util.Map<String, Object> body) {
        ...
        // Zotero 的行为：标注导出到笔记时自动带上引文，否则笔记里的引文只能手抄。
        // style 可由调用方指定，默认 APA。
        ...
        for (Annotation a : annotations) {
            ...
            // ^ann-N 兼容 Obsidian 块引用；[[paper:..#ann-..]] 是应用内可解析的回跳链接，
            // 没有它「双向跳转」只有去程没有回程。

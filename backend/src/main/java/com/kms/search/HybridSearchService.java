 ...
 *
 * 流程：
 * 1. Metadata 过滤（year range / tag / favorite）—— 缩小候选集
 * 2. 关键词命中（ILIKE title / authors / abstract）—— 精确匹配
 * 3. 语义命中（pgvector 余弦距离）—— 概念匹配
 * 4. RRF（Reciprocal Rank Fusion）合并两路结果 —— 综合排序
 *
 * RRF 公式：score = Σ 1/(k + rank_i)，k=60 是业界常用值。
 * 关键词排名靠前时贡献大，语义排名靠前时也贡献大，两路都靠前时分数最高。
 */
@Service

...
public class LlmClientFactory {
    ...
    /** parse currentEnabled  embedding Model(capability='embedding'), provideCallpartyRead model_id / Dimension.  */
    public LlmModel embeddingModel(Long modelId) {
        return modelService.resolveEmbedding(modelId);
    }

    /** useParsedok embedding Model directly generates vector, Avoid per-item reparse in batch.  */
    ...
    public ModelTestResult legacyTestModel(Long id) {
        ...
        if (Boolean.TRUE.equals(result.get("ok"))) {
            return ModelTestResult.ok(id, String.valueOf(id), "connectSuccess, Model returns: " + result.get("message"));
        }

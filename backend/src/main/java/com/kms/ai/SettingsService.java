...
public class SettingsService {
    ...
    public void deleteModel(Long id) {
        ...
        if (model.isDefaultModel()) {
            throw new ApiException(HttpStatus.CONFLICT, "Default Model Cannot Delete, Please set another default model first. ");
        }
        modelRepository.delete(model);
    }

    private void applyRequest(ModelConfig model, ModelConfigRequest request, boolean creating) {
        if (request.getName() != null || creating) model.setName(nonBlank(request.getName(), "Unnamed Model"));

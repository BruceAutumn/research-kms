...
public class WorkflowService {
    ...
    @Transactional public WorkflowDtos.WorkflowDto create(WorkflowDtos.WorkflowRequest req){ Workflow w=new Workflow(); w.setUserId(CurrentUser.ID); w.setName(req.name==null||req.name.isBlank()?"未命名 Workflow":req.name.trim()); w.setDescription(req.description); Workflow saved=workflowRepository.save(w); replaceSteps(saved.getId(), req.steps); return toDto(saved,true); }
